 ...
 * useSession() hook and getSession() function.
 */
 ...
 */
async function refreshAccessToken(token: Session): Promise<Session> {
 ...
 * 2. **Active Use**: Access token used for API calls (valid ~1 hours)
...
export const { onRequest, useSession, useSignIn, useSignOut } = QwikAuth$(
	() => ({
		 ...
		 * http://localhost:5173/auth/signin?error=AccessDenied
		 * http://localhost:5173/auth/signin?error=Configuration
		 * http://localhost:5173/auth/signin?error=OAuthAccountNotLinked
		 *
		 * # Error page with different error types
		 * http://localhost:5173/auth/error?error=Configuration
		 * http://localhost:5173/auth/error?error=AccessDenied
		 * http://localhost:5173/auth/error?error=Verification
		 * ```
		 *
		...
		callbacks: {
			...
			async jwt({ token, account, user }) {
				...
				// Refresh the token
				return refreshAccessToken(token as Session);
			},
			 ...
			 * that your application code can access via useSession() or getSession().
			 *
			 *
			 * ## Security Note
			 *
			...
			async session({ session, token }) {
				...
				customSession.idToken = (token as Session).idToken;
				customSession.accessToken = (token as Session).accessToken;
				customSession.error = (token as Session).error;
